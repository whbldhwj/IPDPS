...
__global__ void calMatrix(
    ...
    __shared__ int sw1[BLOCK_SIZE+1];
    __shared__ int sw2[BLOCK_SIZE+1];
    __shared__ int sw3[BLOCK_SIZE+1];
    sw1[tx+1] = 0;
    sw2[tx+1] = 0;
    sw3[tx+1] = 0;
    if (tx == 0){
...
//    __shared__ btrack_t btrack_tile[BLOCK_SIZE][BLOCK_SIZE];
...
//    int ref_offset = pairIter*MAX_REF_LEN;
...
//        int gap_v_offset = pairIter*PADDED_ALT_LEN;
        ...
        for (int diag = 0; diag < BLOCK_SIZE; diag++)
        {
            ...
            if (tx <= diag){
                ...
                swLeft = sw2[t_index_x+1];
                swUp = sw2[t_index_x+1-1];
                swLeftUp = sw3[t_index_x+1-1];

                if (row_tile > 0 && tx == 0){
//                    swUp = sw_lastrow[pairIter*PADDED_ALT_LEN + t_index_y + 1];
//                    swLeftUp = sw_lastrow[pairIter*PADDED_ALT_LEN + t_index_y];
                ...
                b_tmp = (btrack_t)kd;
                b_tmp = (sw_tmp == step_right)? -(btrack_t)ki: b_tmp;
                b_tmp = (sw_tmp == step_diag)? 0: b_tmp;

                sw1[t_index_x+1] = sw_tmp;
                btrack_tile[t_index_x][t_index_y] = b_tmp;

                if (row_tile < nrow_block-1){
                    if (tx == BLOCK_SIZE-1){
//                        sw_lastrow[pairIter*PADDED_ALT_LEN + t_index_y + 1] = sw_tmp;
...
//            btrack[sw_offset + row_tile*BLOCK_SIZE*PADDED_ALT_LEN + PADDED_ALT_LEN + ty*PADDED_ALT_LEN + 1 + tx] = btrack_tile[ty][tx];
        ...
        for (int block = 0; block < ncol_block-1; block++){
            ...
            for (int diag = BLOCK_SIZE + block*BLOCK_SIZE; diag < diagBound; diag++){
                ...
                sw1[t_index_x+1] = sw_tmp;
                btrack_tile[t_index_x][v_ind - (BLOCK_SIZE - tx)] = b_tmp;
            ...
            for (int ty = 0; ty < BLOCK_SIZE; ty++){
//                btrack[sw_offset + row_tile*BLOCK_SIZE*PADDED_ALT_LEN + PADDED_ALT_LEN + ty*PADDED_ALT_LEN + block*BLOCK_SIZE + BLOCK_SIZE - ty + tx + 1] = btrack_tile[ty][tx];
                btrack[ty*PADDED_ALT_LEN + (block+1)*BLOCK_SIZE - ty + tx] = btrack_tile[ty][tx];
            }
        }
        ...
        for (int diag = alt_len; diag < alt_len + BLOCK_SIZE - 1; diag++)
        {
            ...
            if (tx >= diagStart){
                ...
                sw1[t_index_x+1] = sw_tmp;
                btrack_tile[t_index_x][t_index_y - alt_len + tx] = b_tmp;
                if (row_tile < nrow_block-1){
                    if (tx == BLOCK_SIZE-1){
                        sw_lastrow[t_index_y + 1] = sw_tmp;
                    }
...
//                btrack[sw_offset + row_tile*BLOCK_SIZE*PADDED_ALT_LEN + PADDED_ALT_LEN + i*PADDED_ALT_LEN + alt_len - i + j + 1] = btrack_tile[i][j];
